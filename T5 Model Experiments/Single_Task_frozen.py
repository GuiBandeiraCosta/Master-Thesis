...
def train_model(config,f,directory):
    ...
    dataset = load_train_data(f,directory)
    ...
    for batch_size in config['batch_sizes']:
        ...
        wandb.init(project="V3", config=config,group=f"{f}_frozen", name=f"{f}_{batch_size}") 
        ...
        model.to(device)
        optimizer = torch.optim.AdamW(filter(lambda p: p.requires_grad, model.parameters()), lr=float(config['lr']))
...
def main():
    ...
    for filename in os.listdir(f"Tasks/{directory}"):
        ...
        if filename == "22eb0ac0_31_full_ver_list":
