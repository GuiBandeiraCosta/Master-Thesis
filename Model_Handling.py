...
class MultiHeadAttention(nn.Module):
    ...
    def scaled_dot_product_attention(self, Q, K, V, mask=None):
        ...
        if mask is not None:
            ...
            # if mask.size(2) == 1:
            #     expanded_mask = mask.expand(-1,-1,mask.size(-1),-1)
                
            #     adjusted_mask = expanded_mask & expanded_mask.transpose(-2,-1)
                
            #     # Assuming the mask might be larger than needed; we slice it to match the attn_scores size
            #     adjusted_mask = adjusted_mask[:, :, :attn_scores.size(-2), :attn_scores.size(-1)]
                
            #     attn_scores = attn_scores.masked_fill(~adjusted_mask, -1e9)
        ...
        # if mask is not None:
        #     fully_masked_rows = (attn_scores == -1e9).all(dim=-1,keepdim=True)
        #     attn_probs = attn_probs.masked_fill(fully_masked_rows,0.0)
