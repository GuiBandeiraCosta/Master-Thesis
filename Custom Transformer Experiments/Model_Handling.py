...
class LearnedPositionalEncoding1D(nn.Module):
    def __init__(self, max_seq_length,d_model):
        ...
        self.positions = torch.arange(0, max_seq_length, device=x.device).unsqueeze(0)
    
    def forward(self, x):
        batch_size, seq_length, _ = x.size()
        
        position_embeddings = self.positional_embeddings(self.positions[:seq_length])
        return x + position_embeddings


class Transformer(nn.Module):
    def __init__(self,is_2D,src_vocab_size, tgt_vocab_size, d_model, num_heads, num_layers, d_ff, max_seq_length, dropout):
        super(Transformer, self).__init__()
        self.shared_embedding = nn.Embedding(src_vocab_size, d_model)
        self.is_2D = is_2D
        if is_2D:
            self.positional_encoding = DynamicPositionalEncoding2D(d_model, max_seq_length)
            print("Using 2D Positional Encoding")
        else:
            self.positional_encoding = PositionalEncoding(d_model, max_seq_length)
            print("Using Default Positional Encoding")
