...
def test_model(transformer, tokenizer, test_loader, config, device):
    ...
    with torch.no_grad():
        test_loader = tqdm(test_loader, desc=f"Evaluation", leave=False)
...
def main():
    ...
    parser.add_argument("--filename", type=str, default="3aa6fb7a/3aa6fb7a_Max40_JustNumber_100K.json")
    ...
    weight_path = "3aa6fb7a/3aa6fb7a_Max40_JustNumber_100K.json_model_weights.pth"
    transformer.load_state_dict(torch.load(weight_path, map_location=device))
    

    test_loader = load_data(config,tokenizer,batch_size=256,start=0,end=1000)
