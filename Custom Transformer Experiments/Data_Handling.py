...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        src, tgt, grid_size = self.data[idx]
        ...
        return torch.tensor(src_padded, dtype=torch.long), torch.tensor(tgt_padded, dtype=torch.long),torch.tensor(grid_size,dtype=torch.int64)

def load_train_data(filepath):
    with open(filepath,"r") as file:
        data = json.load(file)
    return [(task["input"], task["output"],task['grid_size']) for task in data["train"]]

def load_test_data(filepath):
    with open(filepath,"r") as file:
        data = json.load(file)
    return [(task["input"], task["output"],task['grid_size']) for task in data["test"]]
