...
def main():
    ...
    parser.add_argument("--filename", type=str, default="testing.json")
    ...
    parser.add_argument("--is_2D", action='store_true', default=False)
    ...
    if config['no_wandb']:
        task_name = config['filename'].split("_")[0]
        name_init = config['filename']
        group_name = config['filename'].split("_")[1:] 
        if config['is_2D']:
            name_init += f"_2D_PE_dmodel{config['d_model']}_DecoderOnly"
            group_name += f"_2D_PE_dmodel{config['d_model']}_DecoderOnly"
        else:
            name_init += f"_Default_PE_dmodel{config['d_model']}_DecoderOnly"
            group_name += f"_Default_PE_dmodel{config['d_model']}_DecoderOnly"
        wandb.init(project="Transformer Model PE Testing Several Tasks",config=config, group=group_name, name=name_init)
