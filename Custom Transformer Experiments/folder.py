...
from LearnedEmbeddings import Transformer
...
def run_experiment(config, device):
    ...
    if config['no_wandb']:
        ...
        group_name = os.path.basename(config['filename'])[9:]
        if config['is_2D']:
            name_init += f"_2D_PE_dmodel{config['d_model']}_RoPE"
            group_name += f"_2D_PE_dmodel{config['d_model']}_RoPE"
        else:
            name_init += f"_Default_PE_dmodel{config['d_model']}_Learned_100K"
            group_name += f"_Default_PE_dmodel{config['d_model']}_Learned_100K"
        config["grouping_tag"] = group_name
        wandb.init(project="Transformer Model PE Testing Several Tasks",config=config, name=name_init)
